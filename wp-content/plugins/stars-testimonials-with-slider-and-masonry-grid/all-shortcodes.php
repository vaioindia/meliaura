            ...
            <table class="wp-list-table widefat fixed striped posts">
                <thead>
                    <tr>
                        ...
                        <th><b><?php echo __("Widget Shortcode", 'stars-testimonials') ?></b></th>
                        <th width="220" class="text-center"><b><?php echo __("Action", 'stars-testimonials') ?></b></th>
                    </tr>
                </thead>
                <tbody>
                <?php
                        ...
                        <tr data-nonce="<?php echo wp_create_nonce("star_testimonial_remove_nonce_".$row->id) ?>" id="row-<?php echo $row->id ?>" data-id="<?php echo $row->id ?>" data-type="<?php echo $row->testimonial_type ?>">
                            ...
                            <td>
                                <div class="copy-shortcode">
                                    <?php echo "[testimonial_stars id={$row->id}]" ?><a title="<?php echo __("Click to copy", 'stars-testimonials') ?>" href="javascript:;"><i class="pst pst-clone"></i></a>
                                </div>
                                <div class="sr-only">
                            ...
                            <td class="action-buttons text-center" >
                                <a class="view-preview" target="_blank" href="<?php echo site_url("wp-admin/edit.php?post_type=stars_testimonial&page=all-shortcodes&task=preview&id=".$row->id) ?>"><span class="badge bg-blue"><i class="pst-view"></i> Preview</span></a>
                                <?php if(current_user_can('edit_posts')) { ?>
                                <a class="update-preview" href="<?php echo site_url("wp-admin/edit.php?post_type=stars_testimonial&page=all-shortcodes&task=edit&id=".$row->id) ?>"><span class="badge bg-green"><i class="pst-pencil"></i> Edit</span></a>
                                <?php } ?>
                                <?php if(current_user_can('delete_posts')) { ?>
                                <a class="remove-preview" href="javascript:;"><span class="badge bg-red"><i class="pst-trash"></i> Remove</span></a>
                                <?php } ?>
                            </td>
                        </tr>
                    <?php } ?>
