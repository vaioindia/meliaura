...
jQuery( document ).ready( function(){
    ...
    if(settings.PRO_BUTTON != "" && jQuery(".post_type_page").val() == "stars_testimonial" && jQuery(".post_status_page").val() == "all") {
        if(jQuery(".no-testimonials-area").length) {
            jQuery(".no-testimonials-area").html(settings.IFRAME_CODE)
        } else if(jQuery("#ajax-response").length) {
    ...
    jQuery(".testimonial-col:not(.has-prow-feature)").on('click', function(){
    ...
    jQuery("#grid-form input[name='testimonial_style']").on('change', function(){
    ...
    jQuery("#grid-form input[name='testimonial_style']").on('click', function(e){
        e.stopPropagation();
    });
    jQuery("#grid-form  .grid-form-row").on('click', function(){
       jQuery(this).find("input[name='testimonial_style']").trigger("click");
    });
    jQuery(".customize-button").on('click', function(){
        setTimeout(function(){
            ...
            boxHtml = jQuery(".sr-only:checked").closest(".grid-form-row").find(".grid-style-box .preview-row:first").html();
            jQuery(".preview-section .preview-inner").html(boxHtml+'<div class="clear"></div>');
            setStyleColor(jQuery("#grid-form .sr-only:checked").val());
            jQuery(window).scrollTop(0);
        }, 100);
    });
    jQuery("#custom-color input[type='radio']").on('change', function(){
    ...
    jQuery('.testimonial-color-picker').on('click', function(){
    ...
    jQuery(".custom-select").on('click', function(e){
        e.stopPropagation();
    ...
    jQuery(".custom-select-box .select-content li").on('click', function(e){
        e.stopPropagation();
    ...
    jQuery("a.reset-button").on('click', function(){
        reserData();
    });
    jQuery("a.submit-button").on('click', function(){
        errorCounter = 0;
        ...
        if(errorCounter == 0 && !jQuery(this).hasClass("loading")) {
            jQuery(this).html('<i class="pst-spin pst-refresh rotating" aria-hidden="true"></i>');
    ...
    jQuery(".view-preview").on('click', function(){
