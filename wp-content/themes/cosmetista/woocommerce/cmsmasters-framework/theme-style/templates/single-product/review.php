		...
		<div class="comment-text cmsmasters_comment_item_cont">
				...
				<h3 class="cmsmasters_comment_item_title" itemprop="author"><?php 
					...
					if ( get_option( 'woocommerce_review_rating_verification_label' ) === 'yes' )
						if ( wc_customer_bought_product( $comment->comment_author_email, $comment->user_id, $comment->comment_post_ID ) )
							echo '<em class="verified">(' . esc_html__( 'verified owner', 'cosmetista' ) . ')</em> ';
				?></h3>
			</div>
			<div class="cmsmasters_comment_item_cont_info">
				<?php cosmetista_woocommerce_rating('cmsmasters_theme_icon_star_empty', 'cmsmasters_theme_icon_star_full', true, $comment->comment_ID); ?>
				<time class="cmsmasters_comment_item_date" itemprop="datePublished" datetime="<?php echo get_comment_date(); ?>"><?php echo esc_html__('on', 'cosmetista') . ' ' . get_comment_date(); ?></time>
			</div>
			<div itemprop="description" class="description cmsmasters_comment_item_content">
				...
				if ($comment->comment_approved == '0') {
					echo '<p class="meta">' . 
						'<em>' . esc_html__( 'Your comment is awaiting approval', 'cosmetista' ) . '</em>' . 
					'</p>';
				}
				?>
			</div>
