...
/* Scripts for Admin */
function cosmetista_theme_admin_scripts() {
	wp_enqueue_script('cosmetista-theme-settings-toggle', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/admin/js/theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('cosmetista-theme-settings-toggle', 'cmsmasters_theme_settings', array( 
		'shortname' => 	'cosmetista' 
	));
}

add_action('admin_enqueue_scripts', 'cosmetista_theme_admin_scripts');



/* Register theme JS Scripts */
function cosmetista_register_theme_js_scripts() {
	...
	wp_localize_script('cosmetista-theme-script', 'cmsmasters_theme_script', $cmsmasters_localize_array);
}

add_action('wp_enqueue_scripts', 'cosmetista_register_theme_js_scripts');



/* Register Theme Navigations */
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'cosmetista'), 
    'footer' => 	esc_html__('Footer Navigation', 'cosmetista'), 
	'top_line' => 	esc_html__('Top Line Navigation', 'cosmetista') 
));
...
function cosmetista_fallback_menu($args) {
	$cmsmasters_option = cosmetista_get_global_options();
	...
	echo '<div class="navigation_wrap">' . "\n" . 
		'<ul id="navigation" class="' . 
			'navigation ' . 
			($cmsmasters_option['cosmetista' . '_header_styles'] == 'default' ? 'mid_nav' : 'bot_nav') . 
		'">' . "\n";
...
function cosmetista_single_project_layout($cmsmasters_layout) {
	if (
		is_singular('project') || 
		is_singular('profile') 
	) {
...
add_filter('cmsmasters_theme_page_layout_filter', 'cosmetista_single_project_layout');



/* Search Form */
function cosmetista_search_form($form) {
	$form = "<div class=\"search_bar_wrap\">
		<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<p class=\"search_field\">
				<input name=\"s\" placeholder=\"" . esc_attr__('Search...', 'cosmetista') . "\" value=\"\" type=\"search\" />
			</p>
...
add_filter('get_search_form', 'cosmetista_search_form');



/* Pagination Prev Text */
function cosmetista_pagination_prev_text() {
	return esc_html__('Previous', 'cosmetista');
}

add_filter('cosmetista_pagination_prev_text_filter', 'cosmetista_pagination_prev_text');



/* Pagination Next Text */
function cosmetista_pagination_next_text() {
	return esc_html__('Next', 'cosmetista');
}

add_filter('cosmetista_pagination_next_text_filter', 'cosmetista_pagination_next_text');
